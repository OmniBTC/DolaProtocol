module lending::rates {
    ...
    public fun calculate_utilization(storage: &mut Storage, token_name: vector<u8>, liquidity: u128): u64 {

        let scale_balance = get_dtoken_scaled_total_supply(storage, token_name);
        let cur_borrow_index = get_borrow_index(storage, token_name);
        let debt = scale_balance * (cur_borrow_index as u128) / (RAY as u128);
        ((debt * (RAY as u128) / (debt + liquidity)) as u64)
    }

    public fun calculate_borrow_rate(storage: &mut Storage, token_name: vector<u8>, liquidity: u128): u64 {
        let utilization = calculate_utilization(storage, token_name, liquidity);
    ...
    public fun calculate_liquidity_rate(storage: &mut Storage, token_name: vector<u8>, borrow_rate: u64, liquidity: u128): u64 {
        let utilization = calculate_utilization(storage, token_name, liquidity);
