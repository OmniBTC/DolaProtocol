...
module add_reward_pool::proposal {
    ...
    const DEPOSIT_WORMHOLE_USDC: u64 = 611185714300 * 7;

    // Borrow wormhole usdc reward
    const BORROW_WORMHOLE_USDC: u64 = 611185714300 * 7;

    // Wormhole usdc pool id
    const WORMHOLE_USDC_POOL_ID: u16 = 8;

    // Deposit sui reward
    const DEPOSIT_SUI: u64 = 1411120000000 * 7;

    // Borrow sui reward
    const BORROW_SUI: u64 = 1411120000000 * 7;

    // Sui pool id
    const SUI_POOL_ID: u16 = 3;

    // 2023-08-14 00:00:00 +UTC
    const START_TIME: u256 = 1691971200;

    // 2023-08-21 00:00:00 +UTC
    const END_TIME: u256 = 1692576000;
    ...
    public fun all_reward(): u64 {
        DEPOSIT_WORMHOLE_USDC + BORROW_WORMHOLE_USDC + DEPOSIT_SUI + BORROW_SUI
    }
    ...
    public entry fun add_description_for_proposal(
        ...
        let description: String = ascii::string(b"Add sui reward pool from 2023-08-14 to 2023-08-21");
    ...
    public entry fun vote_porposal(
        ...
        if (option::is_some(&governance_cap)) {
            ...
            let borrow_wormhole_usdc_reward: Coin<SUI> = coin::from_balance(
                balance::split(&mut escrow_reward.reward, BORROW_WORMHOLE_USDC),
                ctx
            );
            ...
            let borrow_sui_reward: Coin<SUI> = coin::from_balance(
                balance::split(&mut escrow_reward.reward, BORROW_SUI),
                ctx
            );
