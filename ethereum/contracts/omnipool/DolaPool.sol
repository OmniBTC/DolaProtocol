...
contract DolaPool {
    ...
    function deposit(
        ...
        bytes memory poolPayload = LibPool.encodeDepositPayload(
            LibDolaTypes.addressToDolaAddress(dolaChainId, token),
            LibDolaTypes.addressToDolaAddress(dolaChainId, tx.origin),
            LibDecimals.fixAmountDecimals(amount, LibAsset.queryDecimals(token)),
            appId,
            appPayload
        );
    ...
    function sendMessage(
        ...
        return LibPool.encodeSendMessagePayload(
            LibDolaTypes.addressToDolaAddress(dolaChainId, tx.origin),
            appId,
            appPayload
        );
